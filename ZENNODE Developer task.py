def apply_discount(cart, rules):
    ...
    for product, quantity in cart["quantities"].items():
        if quantity > 10:
            discounts.append(("bulk_5_discount", cart["prices"][product] * 0.05))
    ...
    if cart["total_quantity"] > 30:
        for product, quantity in cart["quantities"].items():
            if quantity > 15:
                discounts.append(("tiered_50_discount", cart["prices"][product] * 0.5 * (quantity - 15)))
...
def main():
    ...
    cart = {"quantities": {}, "prices": products, "total": 0, "total_quantity": 0, "gift_wraps": {}}
    ...
    discount_name, discount_amount = apply_discount(cart, products)
    shipping_fee, gift_wrap_fee = calculate_shipping_and_gift_fee(cart)

    print("\nOrder Summary:")
    for product, quantity in cart["quantities"].items():
        print(f"{product}: {quantity} units - ${quantity * products[product]}")
